# BAD: attempting scan of open nodes instead of keeping sorted
# 2898.1 vs 1981.99 
...
def find_path_astar(target_x, target_y):
	...
	while len(open) > 0:
		current_index = open[0] # smallest estimated distance
		current_node = nodes[current_index]
		for i in range(len(open) - open_index - 1):
			test_index = open[i+1]
			test_node = nodes[test_index]
			if test_node[2] < current_node[2]:
